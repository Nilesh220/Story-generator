...
trimmed_dir = os.path.join(os.getcwd(), 'scratch', 'trimmed_walkin_stories')
...
clean_folder(cropped_dir)
clean_folder(trimmed_dir)
...
# 2. Trim top ~12% of each walkin story image to remove username/profile headers
print("Trimming top headers (usernames) from story images...")
for f in image_files:
    src_path = os.path.join(source_dir, f)
    dst_path = os.path.join(trimmed_dir, f)
    try:
        with Image.open(src_path) as im:
            w, h = im.size
            # Crop top 12% to remove username header
            top_crop = int(h * 0.12)
            trimmed = im.crop((0, top_crop, w, h))
            trimmed.save(dst_path, quality=95)
...
print("Launching browser generator...")
with sync_playwright() as p:
    ...
    for i, img_filename in enumerate(image_files, 1):
        ...
        # Use trimmed story image relative URL
        img_rel_url = f"scratch/trimmed_walkin_stories/{img_filename}"
        ...
        if i % 20 == 0 or i == len(image_files):
            print(f"[{i:03d}/{len(image_files):03d}] Generated: {out_filename} -> Crop: {style['name']}")

    browser.close()

print(f"\n All {len(image_files)} walkin story screenshots successfully generated and cropped without username headers!")
